...
public class SkipList<K extends Comparable<AirObject>, E> {
     ...
     * @return key of item being searched, or null if not there
     */
    public K find(E key) {
    ...
    /**
     * generated a random level for an object being inserted
     * 
     ...
     * @param key of object inserted
     * @param name value of object inserted 
    ...
    public boolean insert(K key, E name) {
        ...
        for (int i = level; i >= 0; i--) { // Find insert position
            while ((x.forward()[i] != null) && 
                    (x.forward()[i].key().compareTo((AirObject) key) < 0)) {
                x = x.forward()[i];
            }
     ...
     * @return true if deletion was successful, false if item was never there
     */
    @SuppressWarnings("unchecked")
    ...
    public void rangePrint(String start, String end) {
        if (end.compareTo(start) > 0) {
            System.out.print("Found these records in "
                    + "the range |" + start + "| ");
